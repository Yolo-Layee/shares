    ...
    async with aiosqlite.connect(db_file) as conn:
        ...
        if is_use_proxy != 0:
            async with conn.execute(
                ...
                "FROM config WHERE id = (SELECT MAX(id) FROM config)"
            ) as cursor:
                proxy_data = await cursor.fetchone()
                proxy_ip, proxy_port, proxy_username, proxy_password = proxy_data
                proxies = f"http://{proxy_username}:{proxy_password}@{proxy_ip}:{proxy_port}"
        ...
        while parsing:
            ...
            try:
                ...
                async with session.get(browse_nft_url, params=params, headers=headers, proxy=proxies) as response:
                    ...
                    data = json.loads(clearesponse.lstrip('\ufeff'))
            ...
            await query.message.reply_text(
                "Режим парсинга успешно включен, начинаем сканирование рынка\n\n"
                "На этом этапе Вы можете идти отдыхать, я отправлю вам сообщение, "
                "в случае ошибки или в случае, если парсер найдет что-нибудь интересное для Вас\n\n"
                "Не забудьте включить уведомления от бота"
            )
            print("[PARSE] Режим парсинга запущен, начинаем просматривать список NFT")
            ...
            for assetid in assets_ids:
                ...
                try:
                    ...
                    async with session.get(parse_url, proxy=proxies) as response:
                        ...
                        if result < 2:
                            print(f"[PARSE] Коэфициент между справедливой и рыночной стоимостью составил - {result} | NFT URL: {parse_url} | Fair Value: {fair_value} ETH | Buy Value: {favorite_buy_value} ETH")
                            continue
                        else:
                            print(f"[PARSE-GODSEND] Коэфициент между справедливой и рыночной стоимостью составил - {result} | NFT URL: {parse_url} | Fair Value: {fair_value} ETH | Buy Value: {favorite_buy_value} ETH")
                            await query.message.reply_text(
...
async def parse_tumbler_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if parsing:
        parsing = False
        await query.message.reply_text("Пожалуйста подождите, для отключения режима парсинга нужно немного времени...")
    else:
        parsing = True
